...
es-secrets:
	podman run --name elastic-helm-charts-certs -i -w /tmp \
		$(ELASTICSEARCH_IMAGE) \
		/bin/sh -c " \
	...
	podman cp elastic-helm-charts-certs:/tmp/elastic-certificates.p12 ./ && \
	podman rm -f elastic-helm-charts-certs && \
